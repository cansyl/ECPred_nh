		...
		if(ecnums.contains("1.-.-.-")){
			...
			for (int i =0; i< ecnums.size(); i++) {  
				...
				Vector<String> pred = new Vector<>();
				  br = new BufferedReader(new FileReader(tempDir+File.separator+"testResult"+File.separator + time +File.separator+ecnums.get(i)+File.separator + ecnums.get(i)+"_preds.txt"));	
					while((line = br.readLine())!=null){
						pred.add(line);
					}
				allPreds.add( pred);
